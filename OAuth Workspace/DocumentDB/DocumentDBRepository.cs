   ...
   public static class DocumentDBRepository<T>
    {
        ...
        // Alleen aanroepen om een nieuwe collection aan te maken in documentDB
        private static async Task CreateDocumentCollectionIfNotExists()
        {
            ...
            catch (DocumentClientException de)
            {
                if (de.StatusCode == HttpStatusCode.NotFound)
                {
                    ...
                    // Enable TTL, maar default bij collection staat het uit. Override TTL in models
                    collectionInfo.DefaultTimeToLive = -1;

                    // Configure collections for maximum query flexibility including string range queries.
                    collectionInfo.IndexingPolicy = new IndexingPolicy(new RangeIndex(DataType.String) { Precision = -1 });

                    // Maak collection met 400 RU/s.
